...
export default class AdminController {
  ...
  static isTeamValid(memberSids: string[], validSids: string[], callback: any) {
    ...
    // loop thru the team members sids, checking if
    // they are contained in the array of valid sids
    async.each(
      memberSids,
        ...
        if (adminIndex !== -1) {
          ...
          if (_.indexOf(adminsFile[adminIndex].teams, teamId) !== -1) {
            ...
            fs.writeFile(path, JSON.stringify(adminsFile, null, 2), function (error: any) {
              if (!error) {
                Log.trace("AdminController::assignTeam(..) - team " + teamId
                  + " was successfully added to " + username + "'s assigned teams.");
                return callback(null);
              } else {
